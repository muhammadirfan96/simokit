        ...
        <table id="datatablesSimple">
            ...
            <tbody>
                <?php foreach ($limas as $row) : ?>
                    <tr>
                        <td>
                            <a style="pointer-events:<?= $row["approved"] == 'n' ? 'none' : ''; ?>;" class="btn btn-sm <?= $row["approved"] == 'n' ? 'btn-danger' : 'btn-primary'; ?> my-1" href="/approved_limas/printLimas/<?= $row["id"]; ?>" role="button" target="_blank"><i class="fas fa-print"></i></a>
                        </td>
                        <td><?= $row["tanggal"]; ?></td>
